...
#' @param prm List of parameters value.
#' @param interv.prm.0 List of parameters value for baseline intervention.
#' @return No object output. Save RData files (to be used by the fit).
explore.cr <- function(cr.mean.vec, scenidx, prm, interv.prm.0) {
    ...
    for(k in seq_along(interv.prm.0)) 
        interv.prm.0[[k]][['interv_start']] <- 0
...
fit.cr.R0 <- function(R0.target, cr.mean.vec, scenidx, prm, interv.prm.0){
