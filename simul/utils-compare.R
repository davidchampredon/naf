...
compare.simul.scen <- function(scen.id) {
	...
	df <- ddply(df0,'key',summarize, 
				m = mean(value), 
				md = median(value),
				q.lo = quantile(value, probs = 0.5-0.95/2),
				q.hi = quantile(value, probs = 0.5+0.95/2))
	...
	print(paste('Full comparison completed in',round((t2-ct0)/60,2),'minutes for scenario',scen.id,'.'))
}