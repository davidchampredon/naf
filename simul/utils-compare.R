...
output.stats.TEST <- function(x, resp.var, do.ageGroup = FALSE) {
    idx <- which(is.nan(x[[resp.var]]) | is.infinite(x[[resp.var]]))
    if(length(idx)>0) x <- x[-idx, ]
    
    namx <- names(x)
    
    sel <- c('interv_cvg_rate', 'interv_target',
             'interv_cvg_max_prop','interv_start', 'interv_efficacy',
             'imm_hum_baseline')
    
    tmp <- which(namx=='rel.d.treat')+1
    sel <- namx[tmp:length(namx)]
    sel <- c(sel,'scenario_id')
    
    x$scenario_id <- as.numeric(as.character(x$scenario_id))
    ...
    # Define Key to identify scenarios that belong to the same group:
    # TO DO: CHANGE THAT MANUAL DEFINITION AND READ FROM A FILE GENERATED BY THE SCENARIO BUILDER!
    colkey <- which(names(z) %in% c('interv_start',
                                    'interv_cvg_rate',
                                    'interv_target',
                                    'interv_cvg_max_prop',
                                    'interv_efficacy',
                                    'imm_hum_baseline'))
    ztmp <- z[,colkey]
    vtmp <- apply(ztmp, 1, paste, collapse="_") 
    z$key <- vtmp
...
figure.2 <- function(zlist.full) {
    ...
    g <- ggplot(DAT, aes(x=interv_start, y=mn, 
                         color=factor(interv_cvg_rate))) +
        geom_line(alpha=0.6, size = 2, aes(linetype=outcome)) +
    ...
    write.csv(x = dat.sav, file='../results/figures/Figure_2.csv')
}
...
process.outputs.TEST <- function(df,dir,file.scen.prm.list) {
    # Retrieve scenarios definitions:
    spl <- read.csv(file.scen.prm.list) # DEBUG::  file.scen.prm.list <- 'scenario-prm-list.csv'
    names(df)[names(df)=='scenario'] <- 'scenario_id' 
    ...
    z.sympt    <- output.stats.TEST(x, resp.var = 'rel.d.sympt',do.ageGroup = FALSE)
    z.hosp     <- output.stats.TEST(x, resp.var = 'rel.d.hosp', do.ageGroup = FALSE)
    z.death    <- output.stats.TEST(x, resp.var = 'rel.d.death',do.ageGroup = FALSE)
    ...
    names(zlist.full) <- c('sympt','hosp', 'death')
