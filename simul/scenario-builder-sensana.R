...
M[['interv_cvg_rate']]     <- c(0.0030, 0.0120)  
M[['interv_target']]       <- c('priority_age5_frailty')
M[['interv_cvg_max_prop']] <- 0.40 
...
#
A <- list()
A[['contactAssort_lambda']]<- c(0.1, 0.05, 0.2)
A[['frailty_sd']]          <- c(0.1, 0.02, 0.2)
A[['imm_cell_max']]        <- c(0.45, 0.1, 0.9)
A[['contact_rate_CV']]     <- c(0.75, 0.3, 1.5)
A[['proba_move']]          <- c(0.98, 0.8)
A[['asymptom_infectiousness_ratio']]<- c(0.1, 0.05, 0.4)
